<script lang="ts">
	...
	export let initialLocation: LocationSuggestion | null = null;
	...
	let currentStep = 0;
	let locationQuery = initialLocation?.label ?? '';
	let selectedLocation: LocationSuggestion | null = initialLocation;
						...
						<div class="space-y-8">
							<div class="flex flex-col gap-5 border-b border-border/70 pb-6 sm:flex-row sm:items-end sm:justify-between">
								...
								<div class="flex items-center gap-3">
									{#each Array.from({ length: steps.length }, (_, index) => index) as index}
										<div class="flex items-center gap-3">
